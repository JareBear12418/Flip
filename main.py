...
class Ui(QMainWindow):
    def __init__(self):
        ...
        if JSON_CONTENTS['settings'][0]['ShowTut'][0] == 'True':
            ...
            QMessageBox.information(
                self, 'How to Play', 'Make all the boxes blue to win!', QMessageBox.Ok, QMessageBox.Ok)
        else:
    ...
    def update_grid_sizes(self):
        ...
        for i, grid_size in enumerate(JSON_CONTENTS['records'][0].keys()):
            ...
            try:
                ...
                grid.setStatusTip(
                    f'Record Time: Undfined    Record Moves: Undfined')
    ...
    def update_time_label(self):
        try:
            record_time = int(
                JSON_CONTENTS['records'][0][f'{self.grid_size_y}x{self.grid_size_x}'][0]['Time'])
            record_moves = int(
                JSON_CONTENTS['records'][0][f'{self.grid_size_y}x{self.grid_size_x}'][0]['Moves'])
            ...
            JSON_CONTENTS["records"][0].update({str(self.grid_size_y) + "x" + str(
                self.grid_size_x): [{"Moves": "Undfined", "Time": "Undefined"}]})
            with open(JSON_FILE, 'w') as f:
                json.dump(JSON_CONTENTS, f, indent=4)
        except ValueError:
            self.timerLabel.setText(
    ...
    def generate_board(self):
        ...
        self.move_history = []
        self.grid_run_time = [[None for y in range(
            self.grid_size_y)] for x in range(self.grid_size_x)]
        self.button_array_list = [[None for y in range(
            self.grid_size_y)] for x in range(self.grid_size_x)]
        count = 0
    ...
    def check_win(self):
        ...
        if count == self.grid_size_x*self.grid_size_y:
            ...
            self.average_time.append(self.curr_time)
            self.save_scores()
            self.curr_time = QtCore.QTime(00, 00, 00)
            self.current_moves = 0
            self.saved_time = 0
            ...
            if self.currently_played == self.game_limit:
                reply = QMessageBox.information(
                    self, 'You average score for 10 games.', f'Your average scores for 10 games is:\n\nAverage time: {sum(self.average_time) / float(len(self.average_time))}\nAverage Score: {sum(self.average_score) / float(len(self.average_score))}', QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
